    ...
    Paging.prototype = {
        ...
        createDom: function() {
            ...
            for (var i = 1; i <= that.options.totalPages; i++) {
                i != 1 ? ulDom += '<li>' + i + '</li>' : ulDom += '<li class="zyx_sel-page">' + i + '</li>';
            }
            that.options.jump ? jumpDom = '<input type="text" placeholder="1" class="zyx_jump-text zyx_jumpText" ><button type="button" class="zyx_jump-button" >跳转</button>' : jumpDom = '';
            content = '<div class="zyx_pagger-box"><button type="button"  class="zyx_turnPage zyx_first-page">首页</button>' +
                '<button class="zyx_turnPage zyx_pre-page">上一页</button>' +
                '<div class="zyx_pageWrap" style="width:' + wrapLength + 'px">' +
                '<ul class="zyx_pageSelect" style="transition:all ' + that.options.slideSpeed + 'ms">' +
                ulDom +
                '</ul></div>' +
                '<button class="zyx_turnPage zyx_next-page">下一页</button>' +
                '<button type="button" class="zyx_last-page ">尾页</button>' +
                jumpDom +
                '<p class="zyx_total-pages">共&nbsp;' +
                that.options.totalPages +
                '&nbsp;页</p>' +
                '<p class="zyx_total-count">' +
        ...
        bindEvents: function() {
        	var that = this;
        	var $el =that.el;
            var pageSelect = $el.find('.zyx_pageSelect'), // ul
                lis = pageSelect.children(), // li的集合
                liWidth = 42, // li的宽度
                totalPages = that.options.totalPages, // 总页数
                pageIndex = that.options.pageNo, // 当前选择的页码
                distance = 0, // ul移动距离
                prePage = $el.find('.zyx_pre-page'),
                nextPage = $el.find('.zyx_next-page'),
                firstPage = $el.find('.zyx_first-page'),
                lastPage = $el.find('.zyx_last-page'),
                jumpBtn = $el.find('.zyx_jump-button'),
                jumpText = $el.find('.zyx_jump-text');
